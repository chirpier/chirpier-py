...
from chirrop.utils import is_valid_api_key, resolve_api_key
...
class TestUtils(unittest.TestCase):
    ...
    def test_resolve_api_key_precedence(self):
        old_env = os.getenv("CHIRROP_API_KEY")
        try:
            os.environ["CHIRROP_API_KEY"] = "chp_from_env"
            self.assertEqual(resolve_api_key("chp_from_options"), "chp_from_options")
            self.assertEqual(resolve_api_key(None), "chp_from_env")
        finally:
            if old_env is None:
                os.environ.pop("CHIRROP_API_KEY", None)
            else:
                os.environ["CHIRROP_API_KEY"] = old_env
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                with open(".env", "w", encoding="utf-8") as dotenv_file:
                    dotenv_file.write("CHIRROP_API_KEY=chp_from_dotenv\n")
